...
const UserDetailPage: React.FC = () => {
  ...
  const [selectedOrderVariants, setSelectedOrderVariants] = useState<ProductVariant[]>([]);
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const searchMatch =
      ...
      order.productVariants.some((productItem: any) =>
        productItem.productVariant.name.toLowerCase().includes(searchText.toLowerCase())
      );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (createdAt: string) => (
        <span>{moment(createdAt).locale("vi").format("DD [tháng] MM YYYY [vào lúc] h:mm A")}</span>
...
<div className="bg-gray-100 py-8  flex justify-center">
  <div className="w-full max-w-full bg-white shadow-lg rounded-lg p-6">
    ...
    <div className="flex flex-col space-y-6">
      {/* User Information */}
      <Card title={<><PhoneOutlined className="mr-2 text-green-500" />User Information</>} bordered={false} className="w-full">
        <ul className="space-y-2">
          <li><strong>Name:</strong> {user.name}</li>
          <li><PhoneOutlined className="mr-2 text-green-500" /><strong>Mobile:</strong> {user.mobile || "Not available"}</li>
          <li><strong>Role:</strong> {user.role}</li>
          <li><strong>Status:</strong> <Badge color={user.active ? "green" : "red"} text={user.active ? "Active" : "Banned"} /></li>
          <li><MailOutlined className="mr-2 text-green-500" /><strong>Email:</strong> {user.email}</li>
          <li><HomeOutlined className="mr-2 text-green-500" /><strong>Address:</strong> {user.address ? `${user.address.street}, ${user.address.city}` : "No address available"}</li>
        </ul>
      </Card>
    ...
    >
      {selectedOrderVariants.length > 0 ? (
        selectedOrderVariants.map((variant) => (
          <div key={variant._id} className="space-y-4 p-4 bg-gray-50 rounded-lg shadow-lg">
            <Space direction="vertical" size="large" style={{ width: '100%' }}>
              ...
              <div className="flex items-center">
                ...
                {variant.color ? (
                  <Tag color="green" icon={<CheckCircleOutlined />} className="font-bold">
                    {variant.color.colorName}
                  </Tag>
                ) : (
                  <Tag color="red" icon={<CloseCircleOutlined />} className="font-bold">
                    N/A
                  </Tag>
                )}
              </div>
    ...
    <div className="mt-6 flex justify-end">
      {user.active ? (
        <Button danger onClick={handleBanUser}>Ban User</Button>
      ) : (
        <Button type="primary" onClick={handleUnbanUser}>Unban User</Button>
      )}
    </div>
  </div>
</div>
